...
osMessageQueueId_t audioQ, motorQ, ledRedQ, ledGreenQ, sensorQ;
...
void tMotorControl() {
	...
	if(rx_p == FW_MASK && state != FORWARD){
			...
			rightreverse(state);
			state = RVRIGHT;
		}else if(rx_p == RUN_CHALLENGE_TWO) {
			sensorDistance = checkDistance();
			challengeTwo(AUTO,sensorDistance);
			state = AUTO;
		}else if(rx_p == RESET_FWD) {
			reset(state);
			state = AUTO;
		}else {
			stopMotors();
...
void tAudio() {
	...
	for(;;){
		...
		if(rx_p == CONNECT) { //Plays startup tune on connection.
			playStart();
			rx_p = RESET;
			osMessageQueuePut(audioQ, &rx_p, NULL, 0);
		} else if(rx_p == THE_END) { //Plays the ending tune after "The End" is pressed.
			playEnd();
			rx_p = INIT_VAR;
			osMessageQueuePut(audioQ, &rx_p, NULL, 0);
		}else if(rx_p != INIT_VAR){ // Otherwise plays theme all the way through (after connection to the end).
			playCoolTheme();
		} 
	}
}
